...
; Gui, +AlwaysOnTop -SysMenu -ToolWindow -Caption +Owner
Gui, Margin, 0, 0
Gui, Font,s16 "YaHei Console"

If Youdao_基本释义<>
{
    Gui, Add, Edit, y0 w300 r1 ReadOnly, %Youdao_keyword% [ %Youdao_音标% ]
    Gui, Add, Edit, w300 h120 ReadOnly, %Youdao_基本释义%
    Gui, Add, Edit, w300 h120 ReadOnly, %Youdao_网络释义%
}
Else
{
    Youdao_音标:=RegExReplace(Youdao_译文,"m)({""translation""\:\["")|(""\],""query""\:.*)")
    Gui, Add, Edit, y0 w300 r1 ReadOnly, %Youdao_音标%
}
Gui, show,,% Youdao_keyword
ComObjCreate("sapi.spvoice").Speak(Youdao_keyword)
Return
...
json(ByRef js, s, v = "") {
    ...
    {
        vs = "
        If (RegExMatch(v, "^\s*(?:""|')*\s*([+\-]?\d+(?:\.\d*)?|true|false|null?)\s*(?:""|')*\s*$", vx) and (vx1 + 0 or vx1 == 0 or vx1 == "true" or vx1 == "false" or vx1 == "null" or vx1 == "nul"))
            vs := "", v := vx1
        StringReplace, v, v, ", \", All
        js := SubStr(js, 1, z := RegExMatch(js, ":\s*", zx, z) + StrLen(zx) - 1) . vs . v . vs . SubStr(js, z + StrLen(x3) + 1)
    }
    Return, j == "false" ? 0 : j == "true" ? 1 : j == "null" or j == "nul" ? "" : SubStr(j, 1, 1) == """" ? SubStr(j, 2, -1) : j
}
